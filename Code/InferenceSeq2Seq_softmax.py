...
def inferenceLSTM(data_dir, seed=422, **kwargs):
    ...
    outputs = LSTM(unit_encoder, return_sequences=True, name='LSTM_En')(encoder_inputs)
    outputs, state_h, state_c = LSTM(unit_encoder, return_state=True, name='LSTM_En_2')(outputs)
    ...
    decoder_dense = Dense(num_decoder_tokens, activation='softmax', name='DenseLay')
    ...
    x_gen = cond[(2 * N) // 3 + 2]
    y_gen = sigs_out[(2 * N) // 3 + 2]

    # start = time.time()
    last_prediction = y_gen[0]
    predictions = [last_prediction]
    n_steps = y_gen.shape[0]
    print(n_steps)
    output_dim = y_gen.shape[0] - 1

    out, last_prediction = predict_sequence(encoder_model, decoder_model, x_gen, n_steps,
