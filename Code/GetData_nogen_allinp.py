...
def get_data(data_dir, seed=422):
    ...
    for i in range(n_train):
            inp_temp = np.array([sine[i], np.repeat(notes[i], T), np.repeat(vels[i], T)])
            all_inp.append(inp_temp.T)
            tar_temp = np.array(signals[i])
    ...
    for i in range(n_train, n_train + n_val):
            inp_temp = np.array(
                [sine[i], np.repeat(notes[i], T), np.repeat(vels[i], T)])
            all_inp.append(inp_temp.T)
            tar_temp = np.array(signals[i])
            all_tar.append(tar_temp.T)
...
if __name__ == '__main__':
    ...
    x, y, x_val, y_val, x_test, y_test, scaler = get_data(data_dir=data_dir)

    data = {'x': x, 'y': y, 'x_val': x_val, 'y_val': y_val, 'x_test': x_test, 'y_test': y_test , 'scaler': scaler}

    file_data = open(os.path.normpath('/'.join([data_dir, 'NotesDatasetPrepared_allinp_sines.pickle'])), 'wb')
