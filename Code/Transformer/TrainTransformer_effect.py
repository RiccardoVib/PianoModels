...
def train_RAMT(data_dir, epochs, seed=422, data=None, **kwargs):
    ...
    if ckpt_flag or inference_flag:
        ...
        graph_signature = [
            ...
            #tf.TensorSpec((None, Z_shape[1], Z_shape[2]), tf.float32)
            tf.TensorSpec((None, T, D), tf.float32),
            tf.TensorSpec((None, 1, D), tf.float32)
        ]
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_num in range(x.shape[0]):
            x_batch = x[batch_num].reshape(1, T, D)
            y_batch = y[batch_num].reshape(1, T, 1)
        ...
        for batch_num in range(x_val.shape[0]):
            x_batch = x_val[batch_num].reshape(1, T, D)
            y_batch = y_val[batch_num].reshape(1, T, 1)
    ...
    if ckpt_flag and not inference_flag:
        if os.path.exists(save_model_best):
            ...
            x_ = x_test[0].reshape(1, T, D)
            y_ = y_test[0].reshape(1, T, 1)
            # Need to make a single prediction for the model as it needs to compile:
            transformer([tf.constant(x_[:, :-1, :].reshape(1, T-1, D), dtype='float32'),
                         tf.constant(x_[:, -1, :].reshape(1, 1, D), dtype='float32')],
                        training=False)
