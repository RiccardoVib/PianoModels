...
def data_preparation(**kwargs):
    ...
    file_dirs = glob.glob(os.path.normpath('/'.join([data_dir, 'PianoDatasetsSingleNoteDuckSuperShort_Kont.wav'])))
    #L = 48000
    ...
    for file in file_dirs:
        ...
        while end:
            ...
            #plt.show()
            nameFig = save_dir + '/Figs_kont/' + str(note) + '_' + str(vel) + '.png'
            plt.savefig(nameFig)
            plt.close()
            ...
            if len(audio) < int(fs*2):
                end = False
        #signal = audio_format.pcm2float(signal)


    file_data = open(os.path.normpath('/'.join([save_dir, 'NotesDatasetSuperShort_Kont.pickle'])), 'wb')
