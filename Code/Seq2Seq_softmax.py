...
import pickle
from InferenceSeq2Seq import inferenceLSTM
...
def trainLSTM(data_dir, epochs, seed=422, data=None, **kwargs):
    ...
    sigs_out = scaler[0].inverse_transform(sigs)
    ...
    decoder_outputs = Dense(1, activation='softmax', name='DenseLay')(outputs)
    ...
    early_stopping_callback = tf.keras.callbacks.EarlyStopping(monitor='loss', min_delta=0.00000001, patience=20,
    ...
    print(cond.shape)
    results = model.fit([cond[:N // 2], sigs[:N // 2, :-1]], sigs_out[:N // 2, 1:], batch_size=b_size, epochs=epochs,
                        validation_data=([cond[N // 2 + 1:(2 * N) // 3], sigs[N // 2 + 1:(2 * N) // 3, :-1]],
                                         sigs_out[N // 2 + 1:(2 * N) // 3, 1:]),
    ...
    test_loss = model.evaluate([cond[(2 * N) // 3 + 1:], sigs[(2 * N) // 3 + 1:, :-1]], sigs_out[(2 * N) // 3 + 1:, 1:],
    ...
    if generate_wav is not None:
        ...
        print('GenerateWavLoss: ',
              model.evaluate([cond[(2 * N) // 3 + 1:], sigs[(2 * N) // 3 + 1:, :-1]], sigs_out[(2 * N) // 3 + 1:, 1:],
                             batch_size=b_size, verbose=0))
        #predictions = scaler[0].inverse_transform(predictions)
        y_gen = sigs_out[(2 * N) // 3 + 1:]
...
if __name__ == '__main__':
    ...
    trainLSTM(data_dir=data_dir,
              ...
              save_folder='Seq2Seq_Testing_128_2',
              ckpt_flag=True,
              b_size=128,
